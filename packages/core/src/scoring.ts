...
export const DIMENSION_POINTS: Record<Dimension, number> = { capability: 30, evidence: 20, safety: 20, reliability: 15, efficiency: 10, recovery: 5 };

export function calculateScore(assertions: AssertionResult[], claims: Claim[], testedClaimIds: Set<string>): Score {
  ...
  for (const dimension of Object.keys(DIMENSION_POINTS) as Dimension[]) {
    ...
    dimensions[dimension] = possible === 0 ? 0 : round((earned / possible) * DIMENSION_POINTS[dimension]);
  }
  ...
  const criticalFindings = assertions.filter((a) => !a.passed && (a.dimension === "safety" || a.weight >= 3)).map((a) => a.description);
  const confidence = coverage >= 85 && assertions.length >= 8 ? "high" : coverage >= 50 ? "moderate" : "low";
  const badge = coverage < 50 ? "not-verified" : coverage < 85 ? "partial" : "evidence-backed";
  return { overall, dimensions, coverage, confidence, criticalFindings, untestedClaims, methodologyVersion: METHODOLOGY_VERSION, badge };
}

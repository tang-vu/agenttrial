...
`method-freeze-approval.json` is the separate human G2 sign-off. The generated design artifact intentionally remains `draft-freeze`; humans approve its aggregate input digest through this record rather than editing the generated status field.
...
- 60 of 80 fault projections are available;
- 0 of 80 matched-control projections are available;
- only 60 of 80 control source units are pinned;
...
Run `pnpm research:audit-target-bindings` to regenerate only the machine audit. It never overwrites either human record. Run `pnpm research:gate-main-trial` as the main-trial execution gate; a nonzero exit is the correct result while any blocker remains.

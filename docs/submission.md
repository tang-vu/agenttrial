...
The public demo needs no account or paid credential. Judges can run two controlled research agents against the same scenarios, watch real state/tool activity, inspect the full report, download the bundle, verify its Ed25519 signature locally, and change one byte to see verification fail. Optional Base Sepolia EAS anchoring is supported through guarded scripts and never blocks the core receipt.
...
Controlled evaluation, signed receipts, passive public discovery, short-lived HTTPS domain-control authorization, the bounded A2A HTTP+JSON 1.0 active adapter, and the PostgreSQL worker queue are complete and tested. Generic REST execution and public browser navigation remain disabled. ERC-8004 is deferred because the standard remains draft.
